...
{ 
    ...
    { 
        ...
        $param = array(); 
        $text = URLEncode(iconv('gbk', 'utf-8',  $text));
        $param['status'] = $text;
	...
	添加勋章
	 */
	function add_medal($uids,$badge_id,$source)
	{
		$param = array(); 
        $param['uids'] = $uids;
		$param['badge_id'] = $badge_id;
		$param['source'] = $source;
		return $this->oauth->post( 'http://api.t.sina.com.cn/badges/app/issue.json' , $param );
	}
	/**
	删除勋章
	http://api.t.sina.com.cn/badges/app/remove
	*/
	function cancel_medal($uids,$badge_id,$source)
	{
		$param = array(); 
        $param['uids'] = $uids;
		$param['badge_id'] = $badge_id;
		$param['source'] = $source;
		return $this->oauth->post( 'http://api.t.sina.com.cn/badges/app/remove.json' , $param );
	}
    /** 
     * 删除微博收藏。 
     *  
	...
	function logout()
	{
		return $this->oauth->get('http://api.t.sina.com.cn/account/end_session.json');
	}
