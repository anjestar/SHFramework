...
function s_db_sql($sql) {
    if (s_bad_string($sql, $sql)
        //为了不浪费资源连接数据库，手动分析sql语句，不用show "select * from user;"
        || false === ( $desc = s_db_desc($sql) )
    ) {
        return false;
    }
    ...
    //分析查询语句，对应到queryAll, queryRow, queryOne语句
    if ($desc["type"] === "select") {
        //查询
        if ($desc["rows"] === true) {
            //列表
        ...
        ////////////////////////////////////////////////////////////////////////////////
        //插入或者更新要执行的动作特别多：分析字段，更新缓存.TODO:
        $ret = s_db_exec($sql);
    }
