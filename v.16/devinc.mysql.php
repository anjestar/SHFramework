...
function &s_db_plink() {
	$dsn = array(
        'phptype'  => "mysql",
        'username' => 'sh245var365cn',
        'password' => 'vJqy6VebyTq9ckx',
        'hostspec' => 'udb.mainone.com',
        'database' => 'sh245var365cn',
		'charset'  => 'UTF8',
	);
    /*
	$dsn = array(
    ...
    if (false === ( $conn = mysql_pconnect($dsn['hostspec'], $dsn["username"], $dsn["password"]) )
...
function &s_db_slink() {
    ...
    if (false === ( $conn = mysql_connect($dsn['hostspec'], $dsn["username"], $dsn["password"]) )
        || false === mysql_select_db($dsn["database"], $conn)
        || false === mysql_query("SET NAMES '{$dsn['charset']}'", $conn)
    ) {
        return false;
